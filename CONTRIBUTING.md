...
どのような理由でそのような設計にしたかを説明するため、そのようにしなかったらどうなるかを含めて、以下で説明します。
ところでこれらの「自然に実装すると陥りがちな罠」についての知識は主に、先発のプロジェクトである [nodchip/OnlineJudgeHelper](https://github.com/nodchip/OnlineJudgeHelper) の利用やこれへのコントリビュートの過程で得られたものです。
...
具体的には、絶対に通らないサンプルケースをそうと気づかず通そうとして頑張り続けて 10 分から最大で 1 時間ほどが失なわれる、という結果になります。
...
もしとん「破壊的変更を避ける」こととは相反しますし、「消すべきかどうか」の判断は難しいですが、うまくやっていきましょう。

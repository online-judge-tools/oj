...
依存ライブラリによりonline-judge-toolsの開発を楽にすることはできますが、セキュリティの観点で懸念があるかもしれません。
厳密にonline-judge-toolsがセキュアか検証するためには、ライブラリ及びそれらの依存ライブラリを全てのバージョンにわたって検証することが必要になります。
到底そんなことは出来ないので、マイナーライブラリの追加はなるべく避けたいです。

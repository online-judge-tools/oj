...
def exec_command(command_str: str, *, stdin: IO[Any], timeout: Optional[float] = None, gnu_time: Optional[str] = None) -> Tuple[Dict[str, Any], subprocess.Popen]:
    ...
    with context as fh:
        ...
        try:
            import locale
            # proc = subprocess.Popen(command.encode().decode(locale.getdefaultlocale()[1]), stdin=stdin, stdout=subprocess.PIPE, stderr=sys.stderr)
            proc = subprocess.Popen(command.encode().decode(), stdin=stdin,

...
''')
    ...
    subparser.add_argument('-i', '--from-file', action='store_true', help='tests each case (specified by -d option or test argument), whose filename is passed to the judge as sys.argv[1]')
...
def run_from_file(args: argparse.Namespace) -> bool:
    # list tests
    if not args.test:
